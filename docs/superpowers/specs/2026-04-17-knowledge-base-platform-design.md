...
├── chunk_count       INT DEFAULT 0                     (保留)
├── filename, status, created_at                        (保留)
└── 其他字段不变
...
**关键设计决策**：RPC **按单个 KB** 调用（`p_kb_id UUID` 单数），多 KB 检索由 Service 层并发 N 次调用再合并。原因：每个 KB 有独立的 `threshold / stage1TopK`，单次 RPC 用数组参数就无法让每个 KB 用自己的阈值。学习项目 KB 数量不大（一个 persona 挂 3-5 个 KB 已经很多），并发 N 次 RPC 的开销可以忽略。
...
**关键变化**：`p_persona_id UUID` → `p_kb_id UUID`（单数）；加 `c.enabled = true` 过滤；返回 `knowledge_base_id` 便于前端展示"命中了哪个 KB"。
...
| RPC 按单 KB 调用，Service 层并发合并 | 每个 KB 有独立 threshold/stage1TopK，数组参数做不到分别生效 |
...
export class KnowledgeChunk {
  ...
  // char_count 是 GENERATED ALWAYS 列，TypeORM 不能写入，必须显式标记只读
  @Column({ name: 'char_count', insert: false, update: false }) charCount: number;
...
// 新
ingestDocument(kbId, filename, content, opts?: {
  mimeType?: string
  fileSize?: number
  category?: string       // 保留 category（当前前端没传，但 schema 还在，不隐性丢）
})

retrieve(kbId: string, query, options)                // 单 KB 检索
retrieveWithStages(kbId: string, query, options)      // 单 KB，用于命中测试页
retrieveForPersona(personaId, query)                  // persona 聚合检索（内部并发调多次 retrieve）
```

**`retrieveForPersona` 合并策略**（并发多 KB）：

1. 查 `persona_knowledge_base` 得到 kbIds 与对应的 `retrieval_config`
2. `Promise.all` 并发：对每个 kb 用**自己的** `retrievalConfig.threshold / stage1TopK` 各调一次 `match_knowledge` RPC（stage1）
3. 把所有 stage1 结果合并，按 similarity 全局排序，去重（同一 chunk 不会重复命中）
4. 截断到全局 stage1TopK（取各 KB 中最大的，或用 persona 级兜底值 20）
5. 统一用 reranker 跑 stage2（持久的 finalTopK 用 persona 级兜底值 5）
6. 返回 stage2

**关键**：stage1 的 threshold/stage1TopK **按 KB 生效**；stage2 的 rerank/finalTopK 是全局策略，不按 KB 区分。这样既尊重了每个 KB 的检索配置，又避免 rerank 被多 KB 语义稀释。

（简单策略，学习项目足够；复杂场景可引入 RRF（Reciprocal Rank Fusion）作为 v2）
...
- 写入**新** `match_knowledge(query_embedding, p_kb_id UUID, ...)` —— 单 KB 签名
- 写入**过渡 shim** `match_knowledge_legacy(query_embedding, p_persona_id UUID, ...)`：内部转发到新表，让 Phase 1 期间老代码（`AgentService` 还没切换）能跑

```sql
-- 过渡 shim，Phase 2 结束时删除
CREATE OR REPLACE FUNCTION match_knowledge_legacy(
  query_embedding  VECTOR(1024),
  p_persona_id     UUID,
  match_threshold  FLOAT,
  match_count      INT
...
RETURNS TABLE (
  id UUID, content TEXT, source TEXT, chunk_index INT,
...
Phase 1 把 `KnowledgeService.retrieveStage1` 暂时改为调 `match_knowledge_legacy`（保老代码可跑）；Phase 2 切到新 RPC 并删 shim。
...
6. **RPC 重写**：`match_knowledge` 签名改为单 `p_kb_id`；写 `match_knowledge_legacy` shim 保 Phase 1 老代码
7. **应用侧**：`AgentService.run` 切到 `retrieveForPersona`，`KnowledgeController` 改新路由，旧路由直接下线，shim RPC 删除
...
**注意**：Phase 1 结束时，数据库已经是新结构，但 RPC 层并存两个函数：
- `match_knowledge(..., p_kb_id UUID, ...)` —— 新签名
- `match_knowledge_legacy(..., p_persona_id UUID, ...)` —— 过渡 shim，通过 `JOIN persona_knowledge_base` 把 persona_id 翻译成 kb_ids，让 `AgentService` 未切换时仍能跑

把 `KnowledgeService.retrieveStage1` 的 `.rpc('match_knowledge', {...})` 临时改为 `.rpc('match_knowledge_legacy', {...})`。Phase 2 把 Agent/Service 切到新 RPC 后再删 shim。
...
3. `KnowledgeService` 方法签名迁移：`retrieve(kbId, ...)` 单 KB + `retrieveForPersona(personaId)` 内部 `Promise.all` 并发多 KB
4. `AgentService.run` 切到 `retrieveForPersona`
5. 删除 `match_knowledge_legacy` shim RPC
6. 为新 API 加 Swagger 注解
7. 老路由 `/knowledge/:personaId/*` 下线或加 410 Gone
