...
- `avatar:speaking_start`
- `avatar:speaking_end`
- `tts:start`
- `tts:end`
- `error`

纯语音模式下，TTS 音频继续走 Binary，但 Binary 负载格式需要统一：
...
`onPlaybackEvent()` 的作用有两层：
- `scope='segment'`：给后端内部 drain 队列用，知道一句话什么时候真正播完
- `scope='turn'`：给 Gateway 转成 WebSocket 事件 `avatar:speaking_start` / `avatar:speaking_end`，驱动前端 `thinking` / `speaking` 状态切换

如果 SDK 不提供稳定的播报生命周期回调，前端就只能靠估算时间猜“是不是开始说话了”，这会直接影响按钮状态、字幕节奏和打断体验，因此这是供应商选型的硬要求。
...
| 打断插话 | `thinking/speaking` 状态再次按下，先本地静音远端播放，再发 `conversation:interrupt`，然后开始新录音 | 中断当前 `turnId`，清空缓冲与播报队列 |
...
| **KnowledgeModule** | 知识库管理：文档上传、入队、worker 执行切分/向量化、Milvus CRUD | KnowledgeService |
| **PersonaModule** | 人设管理：角色 CRUD、语音/形象配置 | PersonaService |
| **AsrModule** | ASR 封装：音频流 → 文字 | AsrService |
| **TtsModule** | 流式 TTS 封装（纯语音模式用） | TtsService |
| **DigitalHumanModule** | 数字人 SDK 封装：会话管理、speak、interrupt、播报生命周期事件 | DigitalHumanService |
...
开工前需要用最小 demo 验证以下 6 个供应商相关的假设，否则实现中途可能需要返工：

| 验证项 | 要验证的假设 | 如果不成立的影响 |
|---|---|---|
| 数字人 SDK 的 `speak()` 是否支持排队 | 连续调用 speak() 时后一句等前一句说完 | 不支持则后端需要自建播报队列 |
| 数字人 SDK 是否支持句级增量 `speak()` | 多次按句调用 `speak()` 能连续播报，不要求整段文本一次提交 | 不支持则数字人模式要降级为整段播报 |
| 数字人 SDK 是否提供稳定的播报生命周期回调 | 能拿到句段结束和 turn 开始/结束事件 | 没有回调则前端状态机和队列调度都只能靠猜 |
