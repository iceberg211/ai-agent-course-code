    ...
    if (hasSkill) {
      ...
      if (!skillRegistry.has(name)) {
        errors.push({
          stepIndex: i,
          field: 'skillName',
          message: `Skill "${name}" 未注册，可用：${skillRegistry
            .getAll()
            .map((s) => s.name)
            .join(', ')}`,
        });
        ...
        if (skill.effect === 'side-effect' && !allowedSkills.has(name)) {
          errors.push({
            stepIndex: i,
            field: 'skillName',
            message: `Side-effect Skill "${name}" 未在允许列表中`,
          });
        }
        if (step.skillInput != null) {
          ...
          if (!result.success) {
            errors.push({
              stepIndex: i,
              field: 'skillInput',
              message: `skillInput 不符合 schema: ${result.error.issues.map((x) => x.message).join('; ')}`,
            });
          }
        }
      }
    ...
    if (hasTool) {
      ...
      if (!toolRegistry.has(name)) {
        errors.push({
          stepIndex: i,
          field: 'toolHint',
          message: `Tool "${name}" 未注册，可用：${toolRegistry
            .getAll()
            .map((t) => t.name)
            .join(', ')}`,
        });
        ...
        if (tool.type === 'side-effect' && !allowedTools.has(name)) {
          errors.push({
            stepIndex: i,
            field: 'toolHint',
            message: `Side-effect Tool "${name}" 未在允许列表中`,
          });
        }
        if (step.toolInput != null) {
          ...
          if (!result.success) {
            errors.push({
              stepIndex: i,
              field: 'toolInput',
              message: `toolInput 不符合 schema: ${result.error.issues.map((x) => x.message).join('; ')}`,
            });
          }
        }
      }
