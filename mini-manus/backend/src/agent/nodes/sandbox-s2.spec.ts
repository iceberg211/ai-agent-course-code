...
function makeRegistries(sandboxEnabled: boolean) {
  const toolReg = new ToolRegistry({ get: jest.fn().mockReturnValue('300000') } as any);
  ...
  skillReg.register({
    ...
    description: '生成代码项目',
    effect: 'side-effect',
    inputSchema: { safeParse: () => ({ success: true }), description: '' } as any,
    outputSchema: {} as any,
...
describe('Sandbox S2 Workflow 结构', () => {
  it('sandbox 未启用时，code_generation workflow 只有 1 步（code_project_generation）', () => {
    ...
    const steps = builder(mockState(), { toolRegistry: toolReg, skillRegistry: skillReg });
      ...
      stepResults: [
        {
          ...
          resultSummary: JSON.stringify({
            files: ['project/package.json', 'project/src/main.tsx', 'project/index.js'],
            file_count: 3,
...
describe('Sandbox S2 evaluator 行为', () => {
  ...
  const mockLlm = {
    withStructuredOutput: jest.fn().mockReturnValue(
      RunnableLambda.from(async () => ({ decision: 'continue', reason: 'mock' })),
    ),
  ...
  it('沙箱 exitCode≠0 → code_execution_failed → evaluator 触发 replan（不是 retry）', async () => {
    const state = mockState({
      ...
      currentPlan: {
        planId: 'plan-1',
        steps: [
          { stepIndex: 0, description: '运行代码', toolHint: 'sandbox_run_node' },
        ],
      },
      retryCount: 0,
      replanCount: 0,
    ...
    const result = await evaluatorNode(
      state, mockLlm, mockCallbacks as AgentCallbacks, mockPublisher,
      'functionCalling', 3, 2,
    );
