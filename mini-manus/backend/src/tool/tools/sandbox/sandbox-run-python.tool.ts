...
@Injectable()
export class SandboxRunPythonTool implements Tool {
  readonly name = 'sandbox_run_python';
  readonly description =
    '在隔离 Docker 沙箱中运行 Python 脚本（无网络、只挂载当前 task workspace），' +
  ...
  async execute(input: unknown): Promise<ToolResult> {
    try {
      ...
      return classifyToolError(err, 'sandbox_run_python failed');
    }
  }
}
