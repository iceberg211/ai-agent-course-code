...
function chunk(id: string, partial?: Partial<KnowledgeChunk>): KnowledgeChunk {
  return {
    id,
...
describe('graph-expand.utils golden', () => {
  it('G0: graphExpand=false 时跳过', () => {
    ...
    expect(decision.reason).toContain('关闭');
  });
  ...
  it('G3: hybrid graph 命中 >= 阈值时跳过二次 expand', () => {
    const docs = Array.from(
      { length: GRAPH_EXPAND_SKIP_HIT_THRESHOLD },
      (_, i) =>
    ...
    expect(decision.skip).toBe(true);
    expect(decision.reason).toContain('跳过二次扩展');
  });
  ...
  it('长中文句会保留句尾实体，不再只取前两个前缀块', () => {
    const terms = extractGraphEntitySearchTerms(
      '请说明示例服务协议里协议终止后的试用数据应如何处理',
    );

    expect(terms).toContain('示例服务协议');
    expect(terms).toContain('协议终止');
    expect(terms).toContain('试用数据');
  });
});
