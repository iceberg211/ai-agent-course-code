...
export class LlmRerankerProvider implements RerankerProvider {
  private readonly logger = new Logger(LlmRerankerProvider.name);
  private readonly model: string;
  private readonly llm: ChatOpenAI;

  constructor(private readonly llmFactory: LlmFactoryService) {
    this.model = this.llmFactory.resolveModel({
  ...
  async rerank(input: RerankInput): Promise<KnowledgeChunk[]> {
    ...
    if (candidates.length > RERANKER_MAX_CANDIDATES) {
      rerankCandidates = candidates.slice(0, RERANKER_MAX_CANDIDATES);
    }
  ...
  private applyScores(
    ...
    const threshold = minScore !== undefined
      ? minScore
      : (isDecimalScale ? RERANKER_MIN_RELEVANCE_SCORE * 0.1 : RERANKER_MIN_RELEVANCE_SCORE);
